...
export async function submitOnboardingAction(data: OnboardingData) {
	...
	const { error: goalError } = await supabase.from('user_goals').upsert({
		...
		current_muscle_mass: validated.current_muscle_mass || null,
		target_muscle_mass: validated.target_muscle_mass || null,
	...
	try {
		...
		planData = await generateWorkoutPlan({
			...
			current_weight: validated.current_weight,
			target_weight: validated.target_weight,
			current_muscle_mass: validated.current_muscle_mass,
			target_muscle_mass: validated.target_muscle_mass,
			gender: validated.gender,
			goal:
				validated.target_weight > validated.current_weight
					? 'bulk_up'
					: 'weight_loss',
		});
	} catch (error) {
		...
		// Fallback Mock Plan
		const isBulking = validated.target_weight > validated.current_weight;
		planData = {
			split: '3_day_split',
			name: isBulking
				? '기본 벌크업 3분할 (Fallback)'
				: '기본 다이어트 3분할 (Fallback)',
