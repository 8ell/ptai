...
export default function OnboardingPage() {
	...
	const handleSubmit = () => {
		setStep(8); // Loading/Analyzing step
			...
			const result = await submitOnboardingAction({
				...
				current_muscle_mass:
					formData.current_muscle_mass === 0
						? null
						: formData.current_muscle_mass,
				target_muscle_mass:
					formData.target_muscle_mass === 0
						? null
						: formData.target_muscle_mass,
			});

			if (result.error) {
				toast.error(result.error);
				setStep(7); // Back to review step
			} else {
	...
	const updateField = (field: string, value: number) => {
		setFormData((prev) => {
			if (field === 'target_weight') {
				targetCustomizedRef.current = true;
				return { ...prev, target_weight: value };
			}

			if (field === 'current_weight') {
				const next = { ...prev, current_weight: value };
				const shouldSyncTarget =
					!targetCustomizedRef.current ||
					prev.target_weight === prev.current_weight;
				if (shouldSyncTarget) {
		...
		<div className='min-h-screen bg-background flex flex-col items-center justify-center p-6 relative overflow-hidden'>
			<div className='w-full max-w-md space-y-8 z-10'>
				...
				<AnimatePresence mode='wait' custom={step}>
					...
					{step === 2 && (
						<motion.div
							...
							<div className='flex gap-3'>
								...
								<Button
									onClick={handleNext}
									className='flex-[2] h-14 text-lg rounded-full'>
									다음
								</Button>
							</div>
						</motion.div>
					...
					{/* Step 5: Current Muscle Mass (Optional) */}
					{step === 5 && (
						<motion.div
							key='step5'
							variants={slideVariants}
							...
							<div className='flex justify-center gap-2 mb-4'>
								<Button
									...
									size='sm'
									onClick={() =>
										updateField('current_muscle_mass', 0)
									}>
									잘 모르겠어요
								</Button>
								<Button
									...
									size='sm'
									onClick={() =>
										updateField('current_muscle_mass', 25)
									} // Default value to start picking
								>
									직접 입력
								</Button>
							</div>
					...
					{/* Step 6: Target Muscle Mass (Optional) */}
					{step === 6 && (
						<motion.div
							key='step6'
							variants={slideVariants}
							...
							<div className='flex justify-center gap-2 mb-4'>
								<Button
									...
									size='sm'
									onClick={() =>
										updateField('target_muscle_mass', 0)
									}>
									목표 없음
								</Button>
								<Button
									...
									size='sm'
									onClick={() =>
										updateField('target_muscle_mass', 25)
									} // Default value to start picking
								>
									직접 입력
								</Button>
						...
						<motion.div
							...
							<div className='bg-card border rounded-2xl p-6 space-y-4 shadow-sm text-left'>
								...
								<div className='flex justify-between items-center'>
									...
									<span className='font-bold text-lg'>
										{formData.current_muscle_mass
											? `${formData.current_muscle_mass} kg`
											: '-'}
									</span>
								</div>
								<div className='flex justify-between items-center'>
									...
									<span className='font-bold text-lg'>
										{formData.target_muscle_mass
											? `${formData.target_muscle_mass} kg`
											: '-'}
									</span>
								</div>
							</div>
							...
							<div className='flex gap-3 pt-4'>
								...
								<Button
									...
									disabled={isPending}
									className='flex-[2] h-14 text-lg rounded-full shadow-lg shadow-primary/30'>
									AI 플랜 생성하기
								</Button>
							</div>
						</motion.div>
