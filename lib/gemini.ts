...
export async function generateWorkoutPlan(userInfo: unknown) {
	if (!GEMINI_API_KEY) {
		throw new Error('GEMINI_API_KEY is not defined in environment variables');
	}

	const prompt = `You are an expert fitness trainer AI. Create a detailed weekly workout plan for a user with the following profile:
